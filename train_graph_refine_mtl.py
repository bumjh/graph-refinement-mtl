"""Standalone graph-refined multi-task learning for JustRAIGS.
...
class JustRAIGSMultiTaskDataset(Dataset):
    def __init__(
        ...
        self.rg_valid_only = bool(rg_valid_only)
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        for label_col in AUX_COLUMNS:
            ...
            if self.rg_valid_only:
                mask *= float(final_label >= 0.5)
            if self.positive_only:
                mask *= float(label >= 0.5)
...
def _make_loader(
    ...
    dataset = JustRAIGSMultiTaskDataset(
        ...
        positive_only=bool(train and getattr(args, "aux_positive_only", False)),
        rg_valid_only=bool(require_targets and getattr(args, "aux_rg_valid_only", False)),
    )
...
def compute_aux_pos_weights(
    ...
    for label_col in AUX_COLUMNS:
        ...
        valid = masks > 0
        if rg_valid_only:
            final_labels = train_df[FINAL_COL].apply(parse_binary_value).astype(float)
            valid &= final_labels >= 0.5
        if positive_only:
            valid &= labels >= 0.5
...
def compute_train_label_priors(
    train_df: pd.DataFrame,
    positive_only: bool = False,
    rg_valid_only: bool = False,
...
def build_loaders(
    ...
    aux_pos_weight = compute_aux_pos_weights(
        ...
        power=args.pos_weight_power,
        positive_only=bool(getattr(args, "aux_positive_only", False)),
        rg_valid_only=bool(getattr(args, "aux_rg_valid_only", False)),
    )
    final_prior, aux_priors = compute_train_label_priors(
        train_df,
        positive_only=bool(getattr(args, "aux_positive_only", False)),
        rg_valid_only=bool(getattr(args, "aux_rg_valid_only", False)),
    )
...
def initialize_graph_refinement(model: GraphRefineMTL) -> None:
    """Start refinement near identity so the graph branch cannot shock the backbone."""
    with torch.no_grad():
        for source, target in zip(model.initial_aux_heads, model.refined_aux_heads):
            target.load_state_dict(source.state_dict())
        final_linear = model.clinical_gcn.message_mlp[-2]
        if isinstance(final_linear, nn.Linear):
            nn.init.zeros_(final_linear.weight)
            nn.init.zeros_(final_linear.bias)
        model.clinical_gcn.task_embeddings.zero_()


def load_weights_only(checkpoint_path: Path, model: nn.Module, device: torch.device) -> None:
    """Load only compatible model weights without resuming optimizer or epoch state."""
    ...
    print(
        f"Warm-start weights -> {checkpoint_path} | loaded={len(compatible)} | "
        f"missing={len(missing)} | unexpected={len(unexpected)}"
    )


def freeze_baseline_modules(model: GraphRefineMTL) -> None:
    """Freeze only the ConvNeXt backbone."""
...
def validate(
    ...
    for batch in progress:
        ...
        batch_probs_aux_init = torch.sigmoid(outputs["aux_init_logits"]).detach().cpu().numpy()
        batch_probs_aux_refined = torch.sigmoid(outputs["aux_refined_logits"]).detach().cpu().numpy()
        ...
        for idx, image_id in enumerate(image_ids_iter):
            ...
            for task_idx, task_name in enumerate(AUX_COLUMNS):
                ...
                row[f"{task_name}_init_prob"] = float(batch_probs_aux_init[idx, task_idx])
                row[f"{task_name}_refined_prob"] = float(batch_probs_aux_refined[idx, task_idx])
            if has_targets:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Load compatible model weights only; optimizer and epoch state are not resumed.",
    )
    parser.add_argument(
        "--graph-warmup-epochs",
        type=int,
        default=0,
        help="Train RG and initial auxiliary heads only for this many epochs before enabling GCN refinement.",
    )
    parser.add_argument(
        "--freeze-baseline",
        action="store_true",
        help="Freeze only the ConvNeXt backbone after loading the baseline checkpoint.",
    )
    parser.add_argument(
        "--aux-positive-only",
        action="store_true",
        help="Train auxiliary losses only on valid positive annotations; validation/test masks remain unchanged.",
    )
    parser.add_argument(
        "--aux-rg-valid-only",
        action="store_true",
        help="Use only observed auxiliary annotations within RG cases; exclude NRG-derived synthetic negatives.",
    )
...
def run_single_stage(args: argparse.Namespace) -> None:
    ...
    if args.warm_start_checkpoint:
        load_weights_only(Path(args.warm_start_checkpoint), model=model, device=device)
    if args.model_variant == "graph_refine_mtl":
        initialize_graph_refinement(model)
    if args.freeze_baseline:
        if args.model_variant != "graph_refine_mtl":
            raise ValueError("--freeze-baseline requires --model-variant graph_refine_mtl")
        freeze_baseline_modules(model)
    ...
    print(
        ...
        f"variant={args.model_variant} | graph_warmup={args.graph_warmup_epochs} | "
        f"freeze_baseline={args.freeze_baseline} | "
        f"aux_positive_only={args.aux_positive_only} | "
        f"aux_rg_valid_only={args.aux_rg_valid_only} | "
        f"graph={'fixed_clinical' if model.use_graph_refinement else 'disabled'} | "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        if args.model_variant == "graph_refine_mtl":
            model.use_graph_refinement = epoch > max(0, args.graph_warmup_epochs)
